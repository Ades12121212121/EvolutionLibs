...
-- Validación de temas
local function validateTheme(theme, name)
    local requiredFields = {
        "Background", "Surface", "Primary", "Secondary", "Accent",
        "Text", "TextSecondary", "Border", "Success", "Warning",
        "Error", "Info", "Hover", "Active", "Disabled", "Focus"
    }
    
    for _, field in ipairs(requiredFields) do
        if not theme[field] then
            error(string.format("[EvolutionLibs] Theme '%s' is missing required field: %s", name, field), 2)
        end
        if typeof(theme[field]) ~= "Color3" then
            error(string.format("[EvolutionLibs] Theme '%s' field '%s' must be Color3", name, field), 2)
        end
    end
    
    if not theme.Transparency or type(theme.Transparency) ~= "table" then
        error(string.format("[EvolutionLibs] Theme '%s' is missing Transparency table", name), 2)
    end
    
    local requiredTransparency = {"Light", "Medium", "Heavy", "Glass"}
    for _, field in ipairs(requiredTransparency) do
        if not theme.Transparency[field] then
            error(string.format("[EvolutionLibs] Theme '%s' is missing Transparency.%s", name, field), 2)
        end
    end
...
-- Validar todos los temas predefinidos
for name, theme in pairs(Designs.Themes) do
    validateTheme(theme, name)
end

-- Sistema de efectos visuales mejorado
Designs.Effects = {
    -- Crear sombra con manejo de errores
    CreateShadow = function(element, theme, intensity)
        if not element or not element:IsA("GuiObject") then
            error("[EvolutionLibs] CreateShadow requires a GuiObject", 2)
        end
        if not theme then
            error("[EvolutionLibs] CreateShadow requires a theme", 2)
        end
        ...
        if not success then
            warn("[EvolutionLibs] Failed to create shadow:", shadow)
            return nil
        end
        return shadow
    end,
...
-- API simplificada mejorada
Designs.Simple = {
    -- Configuración rápida con validación
    Setup = function(themeName, options)
        themeName = themeName or "Dark"
        options = options or {}
        
        if not Designs.Themes[themeName] then
            error(string.format("[EvolutionLibs] Theme '%s' not found", themeName), 2)
        end
        ...
        for key, value in pairs(options) do
            if Config[key] ~= nil then
                if type(Config[key]) ~= type(value) then
                    error(string.format("[EvolutionLibs] Invalid type for option '%s'", key), 2)
                end
                Config[key] = value
            end
        end
    ...
    -- Aplicar tema global con validación
    ...
    -- Generar tema personalizado con validación
    CustomTheme = function(baseColor, name)
        if typeof(baseColor) ~= "Color3" then
            error("[EvolutionLibs] baseColor must be Color3", 2)
        end
        
        name = name or "Custom"
        if Designs.Themes[name] then
            warn(string.format("[EvolutionLibs] Theme '%s' already exists, will be overwritten", name))
        end
        
        local success, customTheme = pcall(function()
        ...
        if not success then
            error(string.format("[EvolutionLibs] Failed to create custom theme: %s", customTheme), 2)
        end
    ...
    IsValidTheme = function(theme)
        return pcall(function() validateTheme(theme, "validation") end)
    end,
    
    GetThemeNames = function()
        local names = {}
        for name in pairs(Designs.Themes) do
    ...
    GetCurrentTheme = function()
        return Designs.Themes[Config.DefaultTheme]
    end
}

-- Debug Mode
function Designs.setDebug(enabled)
    Designs._config.DEBUG = enabled
end
